hl.on("hyprland.start", function ()
    ...
    local f = io.open(HOME .. "/.config/secureblue/settings/wallpaper-app", "r")
    if f then
    ...
    -- Start listeners
    hl.exec_cmd("~/.config/secureblue/listeners.sh --startall")
    ...
    -- Restore wallpaper (skip for quickshell — handled inside secureblue-autostart)
    if wallpaper_app ~= "quickshell" then
        hl.exec_cmd("~/.config/secureblue/scripts/secureblue-wallpaper-app --restore")
    end

    -- Autostart scripts
    hl.exec_cmd("~/.config/secureblue/scripts/secureblue-autostart > ~/.mydotfiles/secureblue-autostart.log 2>&1")
