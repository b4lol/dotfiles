...
// comes from secureblue-check-system-updates (the same script the Waybar module
// uses), polled every 30 minutes. The module hides itself completely while no
// updates are available; clicking it launches the SECUREBLUE update routine.
Rectangle {
    id: updates
    ...
    function activate(): void {
        Quickshell.execDetached(["bash", "-c",
            Quickshell.env("HOME") + "/.config/secureblue/settings/installupdates.sh"])
    }
    ...
    Process {
        id: updatesProc
        command: ["bash", "-c",
            Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-check-system-updates"]
        running: true
