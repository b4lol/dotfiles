...
PanelWindow {
    ...
    Process {
        command: ["bash", "-c", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-command-exists hyprmod"]
    ...
    // --- REUSABLE COMPONENTS ---
    component SECUREBLUEMenuItem: MenuItem {
        id: control
        contentItem: Text {
            text: control.text
    ...
    component SECUREBLUEButton: Button {
    ...
    component SECUREBLUESwitch: Switch {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                ActionIcon {
                    ...
                    onClicked: {
                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-toggle-theme"])
                    }
                }
                ...
                ActionIcon {
                    ...
                    onClicked: {
                        root.isOpen = false
                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/secureblue/settings/hyprpicker.sh"])
                    }
                }
            ...
            RowLayout {
                ...
                SECUREBLUEButton {
                    ...
                    onClicked: {
                        root.isOpen = false
                        // Quickshell.execDetached(["kitty", "--class", "dotfiles-floating", "-e", "secureblue-dotfiles-settings", "com.secureblue.dotfiles"])
                        Quickshell.execDetached(["bash", "-c", "qs -p " + Quickshell.env("HOME") + "/.local/share/secureblue-dotfiles-settings/quickshell ipc call settings toggle"])
                    }
                }
                SECUREBLUEButton {
                    text: "HyprMod"
            ...
            ScrollView {
                ...
                ColumnLayout {
                    ...
                    // --- STATUS BAR ENGINE ---
                    // Select which bar SECUREBLUE OS uses. The choice is persisted to
                    // ~/.config/secureblue/settings/statusbar (read by everything else
                    ...
                    RowLayout {
                        ...
                        SECUREBLUESwitch {
                            ...
                            Process {
                                command: ["bash", "-c", "sb=$(tr -d '[:space:]' < ~/.config/secureblue/settings/statusbar 2>/dev/null); [ \"$sb\" = quickshell ] && echo 1 || echo 0"]
                                running: root.isOpen
                                stdout: StdioCollector {
                                    onStreamFinished: {
                            ...
                            onClicked: {
                                ...
                                let cmd = checked
                                    ? "echo quickshell > ~/.config/secureblue/settings/statusbar; qs ipc call statusbar enable; touch ~/.config/secureblue/settings/waybar-disabled; " + Quickshell.env("HOME") + "/.config/waybar/launch.sh"
                                    : "echo waybar > ~/.config/secureblue/settings/statusbar; rm -f ~/.config/secureblue/settings/waybar-disabled; " + Quickshell.env("HOME") + "/.config/waybar/launch.sh; qs ipc call statusbar disable"
                    ...
                    // --- STATUS BAR ---
                    // Single toggle for whichever bar SECUREBLUE OS is configured to
                    // use (read from ~/.config/secureblue/settings/statusbar): either
                    ...
                    RowLayout {
                        ...
                        SECUREBLUESwitch {
                            ...
                            Process {
                                ...
                                command: ["bash", "-c", "sb=$(tr -d '[:space:]' < ~/.config/secureblue/settings/statusbar 2>/dev/null); [ -n \"$sb\" ] || sb=waybar; if [ \"$sb\" = quickshell ]; then f=~/.config/secureblue-statusbar/statusbar.json; [ -f \"$f\" ] || f=~/.config/secureblue/settings/statusbar.json; grep -q '\"enabled\"[[:space:]]*:[[:space:]]*false' \"$f\" && s=0 || s=1; else test -f ~/.config/secureblue/settings/waybar-disabled && s=0 || s=1; fi; echo \"$sb $s\""]
                                stdout: StdioCollector {
                                    onStreamFinished: {
                            ...
                            onClicked: {
                                ...
                                let cmd = activeBar === "quickshell"
                                    ...
                                    : (checked ? "rm -f ~/.config/secureblue/settings/waybar-disabled; " + Quickshell.env("HOME") + "/.config/waybar/launch.sh"
                                               : "touch ~/.config/secureblue/settings/waybar-disabled; " + Quickshell.env("HOME") + "/.config/waybar/launch.sh")
                        ...
                        SettingsWheel {
                            ...
                            Menu {
                                ...
                                // secureblue-statusbar override file to edit; the shipped
                                // statusbar.json is not meant to be edited directly.
                                property bool overrideExists: false
                                Process {
                                    command: ["bash", "-c", "[ -f ~/.config/secureblue-statusbar/statusbar.json ] && echo 1 || echo 0"]
                                    running: root.isOpen
                                    stdout: StdioCollector {
                                        onStreamFinished: {
                                ...
                                SECUREBLUEMenuItem { text: "Reload Status Bar"; onClicked: {
                                        // Reads the settings file and reloads the
                                        // matching bar.
                                        Quickshell.execDetached(["bash", "-c", "~/.config/secureblue/scripts/secureblue-reload-statusbar"])
                                    }
                                }
                                SECUREBLUEMenuItem {
                                ...
                                SECUREBLUEMenuItem {
                                    ...
                                    onClicked: {
                                        root.isOpen = false
                                        Quickshell.execDetached(["gnome-text-editor", Quickshell.env("HOME") + "/.config/secureblue/settings/waybar-quicklinks.json"])
                                    }
                                }
                                SECUREBLUEMenuItem {
                                    text: "Edit Settings"
                                    ...
                                    onClicked: {
                                        root.isOpen = false
                                        // Edit the master file: the secureblue-statusbar override when it
                                        // exists, otherwise the shipped statusbar.json.
                                        Quickshell.execDetached(["bash", "-c", "f=~/.config/secureblue-statusbar/statusbar.json; [ -f \"$f\" ] || f=~/.config/secureblue/settings/statusbar.json; ~/.config/secureblue/settings/editor.sh \"$f\""])
                                    }
                                }
                            }
                        }
                    ...
                    RowLayout {
                        ...
                        SECUREBLUESwitch {
                            ...
                            // in the master file: the secureblue-statusbar override when it
                            // exists, otherwise the shipped statusbar.json. A missing
                            // file or flag counts as off.
                            Process {
                                command: ["bash", "-c", "f=~/.config/secureblue-statusbar/statusbar.json; [ -f \"$f\" ] || f=~/.config/secureblue/settings/statusbar.json; grep -q '\"alwaysExpanded\"[[:space:]]*:[[:space:]]*true' \"$f\" && echo 1 || echo 0"]
                                running: root.isOpen
                                stdout: StdioCollector {
                                    onStreamFinished: {
                    ...
                    RowLayout {
                        ...
                        SECUREBLUESwitch {
                            ...
                            property bool ready: false
                            Process {
                                command: ["bash", "-c", "test -f ~/.config/secureblue/settings/dock-disabled && echo 0 || echo 1"]
                                running: root.isOpen
                                stdout: StdioCollector {
                                    onStreamFinished: {
                            ...
                            onClicked: {
                                if (!ready) return;
                                let fileCmd = checked
                                ? "rm -f ~/.config/secureblue/settings/dock-disabled"
                                : "touch ~/.config/secureblue/settings/dock-disabled"
                    ...
                    RowLayout {
                        ...
                        SECUREBLUESwitch {
                            ...
                            property bool ready: false
                            Process {
                                command: ["bash", "-c", "test -f ~/.config/secureblue/settings/dock-autohide && echo 1 || echo 0"]
                                running: root.isOpen
                                stdout: StdioCollector {
                                    onStreamFinished: {
                            ...
                            onClicked: {
                                if (!ready) return;
                                let fileCmd = checked
                                ? "mkdir -p ~/.config/secureblue/settings && touch ~/.config/secureblue/settings/dock-autohide"
                                : "rm -f ~/.config/secureblue/settings/dock-autohide"
                    ...
                    RowLayout {
                        ...
                        SECUREBLUESwitch {
                            ...
                            property bool ready: false
                            Process {
                                command: ["bash", "-c", "test -f ~/.config/secureblue/settings/gamemode-enabled && echo 0 || echo 1"]
                                running: root.isOpen
                                stdout: StdioCollector {
                                    onStreamFinished: {
                    ...
                    RowLayout {
                        ...
                        SECUREBLUESwitch {
                            ...
                            property bool ready: false
                            Process {
                                command: ["bash", "-c", "test -f ~/.config/secureblue/settings/hide-fastfetch && echo 1 || echo 0"]
                                running: root.isOpen
                                stdout: StdioCollector {
                                    onStreamFinished: {
                            ...
                            onClicked: {
                                if (!ready) return;
                                Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-toggle-fastfetch"])
                            }
                        }
                        Item { implicitWidth: 28 }
                    }
                    ...
                    RowLayout {
                        ...
                        ActionIcon {
                            ...
                            onClicked: {
                                root.isOpen = false
                                Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-wallpaper-app"])
                            }
                        }
                    }
                    ...
                    RowLayout {
                        ...
                        ActionIcon {
                            ...
                            onClicked: {
                                root.isOpen = false
                                Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/secureblue/themes/themes.sh"])
                            }
                        }
                        SettingsWheel {
                            ...
                            Menu {
                                ...
                                background: Rectangle { color: Theme.background; border.color: Theme.primary; border.width: 1; radius: 8 }
                                SECUREBLUEMenuItem { text: "Set GTK Theme"; onClicked: {
                                        root.isOpen = false
                                        Quickshell.execDetached(["nwg-look"])
                                    }
                                }
                                SECUREBLUEMenuItem { text: "Set QT Theme"; onClicked: {
                                        root.isOpen = false
                                        Quickshell.execDetached(["qt6ct"])
                                    }
                                }
                                SECUREBLUEMenuItem { text: "Refresh GTK Theme"; onClicked: {
                                        root.isOpen = false
