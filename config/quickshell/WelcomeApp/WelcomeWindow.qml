...
FloatingWindow {
    ...
    component SECUREBLUEMenuSeparator: MenuSeparator {
        ...
        MenuBar {
            ...
            Menu {
                ...
                SECUREBLUEMenuItem { 
                    text: qsTr("Network");
                    onClicked: { 
                        Quickshell.execDetached(["kitty", "--class", "dotfiles-floating", "-e", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-network"])
                    }
                }    
                SECUREBLUEMenuItem { 
                    text: qsTr("Bluetooth");
                ...
                SECUREBLUEMenuItem { 
                    ...
                    onClicked: {
                        Quickshell.execDetached(["qs", "-p", Quickshell.env("HOME") + "/.local/share/secureblue-dotfiles-settings/quickshell", "ipc", "call", "settings", "toggle"])
                    }
                }
                SECUREBLUEMenuItem { 
                    ...
                    onClicked: { 
                        if (root.isHyprlandSettingsInstalled) {
                            ...
                            Quickshell.execDetached(["kitty", "--class", "dotfiles-floating", "-e", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-install-hyprmod"])
                        }
                    }
                }
                background: Rectangle {
            ...
            Menu {
                ...
                SECUREBLUEMenuItem { 
                    ...
                    onClicked: { 
                        Quickshell.execDetached(["kitty", "--class", "dotfiles-floating", "-e", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-install-sddm"]) 
                    }
                }
                SECUREBLUEMenuItem { 
                    text: qsTr("Network Manager Applet");
                    onClicked: { 
                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-toggle-nmapplet"])
                    }
                }
                SECUREBLUEMenuItem { 
                    text: qsTr("Change Shell");
                    onClicked: { 
                        Quickshell.execDetached(["kitty", "--class", "dotfiles-floating", "-e", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-change-shell"])
                    }
                }
                SECUREBLUEMenuItem { 
                    text: qsTr("System Info") 
                    onClicked: { 
                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/secureblue/scripts/secureblue-hyprsysteminfo"])
                    }
                }
                SECUREBLUEMenuSeparator {}
                SECUREBLUEMenuItem { 
            ...
            Menu {
                ...
                SECUREBLUEMenuItem { text: qsTr("SECUREBLUE OS Homepage"); onClicked: { 
                    Quickshell.execDetached(["xdg-open", "https://secureblue.com/os/"])
                    }
                }
                SECUREBLUEMenuItem { text: qsTr("SECUREBLUE OS GitHub"); onClicked: { 
                    Quickshell.execDetached(["xdg-open", "https://github.com/secureblue/dotfiles"]) 
                    } 
                }
                SECUREBLUEMenuItem { text: qsTr("SECUREBLUE OS Changelog"); onClicked: { 
                    Quickshell.execDetached(["xdg-open", "https://github.com/secureblue/dotfiles/blob/main/CHANGELOG.md"]) 
                    } 
                }
                SECUREBLUEMenuItem { text: qsTr("SECUREBLUE YouTube Channel"); onClicked: { 
                    Quickshell.execDetached(["xdg-open", "https://www.youtube.com/channel/UC0sUzmZ0CHvVCVrpRfGKZfw"]) 
                    } 
                }
                SECUREBLUEMenuItem { text: qsTr("Get more Wallpapers"); onClicked: { 
                    Quickshell.execDetached(["xdg-open", "https://github.com/secureblue/wallpaper"]) 
                    } 
                }
                SECUREBLUEMenuSeparator {}
                SECUREBLUEMenuItem { text: qsTr("Hyprland Homepage"); onClicked: { 
                    Quickshell.execDetached(["xdg-open", "https://github.com/secureblue/wallpaper"]) 
                    } 
                }
                SECUREBLUEMenuItem { text: qsTr("Hyprland Wiki"); onClicked: { 
                    Quickshell.execDetached(["xdg-open", "https://github.com/secureblue/wallpaper"]) 
                    } 
                }
                SECUREBLUEMenuItem { text: qsTr("Update SECUREBLUE OS"); onClicked: { 
                    Quickshell.execDetached(["xdg-open", "https://secureblue.com/os/getting-started/update"]) 
                    } 
                }
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Switch {
                        ...
                        Process {
                            command: ["bash", "-c", "test -f ~/.cache/secureblue-welcome-autostart && echo exists || echo missing"]
                            running: root.visible
                            stdout: StdioCollector {
                                onStreamFinished: {
                        ...
                        onClicked: {
                            if (!ready) return;
                            if (checked) {
                                Quickshell.execDetached(["rm", "-f", Quickshell.env("HOME") + "/.cache/secureblue-welcome-autostart"])
                            } else {
                                Quickshell.execDetached(["touch", Quickshell.env("HOME") + "/.cache/secureblue-welcome-autostart"])
                            }
                        }
                    }
                }                
