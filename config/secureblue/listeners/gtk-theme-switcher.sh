...
apply_theme() {
    ...
    if [[ "$THEME_PREF" == "1" || "$THEME_PREF" == "true" ]]; then
        ...
        $MATUGEN_BIN image $(cat ~/.cache/secureblue/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "dark"

        # Update Quickshell theme
        qs ipc call theme-manager reload
        echo "Quickshell Theme updated"

        # Update SECUREBLUE Dotfiles Settings theme
        qs -p $HOME/.local/share/secureblue-dotfiles-settings/quickshell ipc call theme-manager reload
        echo "SECUREBLUE Dotfiles Settings Theme updated"
        ...
        $MATUGEN_BIN image $(cat ~/.cache/secureblue/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "light"
